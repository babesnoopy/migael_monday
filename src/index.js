...
app.get('/debug/find-task', (req, res) => {
  const q = req.query.q || '';
  res.json(db.all(`SELECT t.id, t.title, t.assignee_id, u.display_name as assignee, t.note FROM tasks t LEFT JOIN users u ON t.assignee_id = u.id WHERE t.title LIKE ?`, [`%${q}%`]));
});
